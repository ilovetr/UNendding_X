...
### 原则1：前后端协同开发
**前/后端任一端改动，必须先评估另一端，必要时同步调整。**

| 场景 | 操作 |
|------|------|
| 后端新增API | 评估前端是否需要调用 |
| 前端新增功能 | 评估后端是否有对应接口 |
| 任何一端完成 | 必须通知对方，确认是否需要配合 |

### 原则2：测试验证标准
**模拟用户行为，端到端验证通过才算测试完成。**
...
### WebSocket 事件教训 (2026-05-03)

**问题**: 报告称 WebSocket 实时推送已完成并测试通过，但实际只有后端代码，前端缺少客户端。

**根因**:
1. 测试不完整 - 只测试后端API发送，没有测试前端接收
2. 概念混淆 - "后端实现了" ≠ "功能完成"
3. 报告不准确 - 应该说"后端已实现，前端待开发"

**正确做法**:
- 检查后端时必须同步检查前端
- 功能状态需明确说明是"部分实现"还是"完整可用"
- 测试必须模拟用户行为进行端到端验证
...
- [x ~~实现 WebSocket 实时消息推送~~] 后端已完成，前端待开发
- [ ] 添加 CLI 命令历史记录
- [ ] 支持配置文件指定默认服务器
- [ ] 添加单元测试覆盖率
- [ ] 实现前端 WebSocket 客户端接收实时推送
